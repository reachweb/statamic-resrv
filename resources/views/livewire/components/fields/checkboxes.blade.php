...
<div {{ $attributes->class(['relative', 'col-span-2' => $field['width'] === 100, 'col-span-1' => $field['width'] === 50,]) }} wire:key={{ $key }}>
    ...
    <div class="flex items-center mb-4">
        <input type="checkbox" wire:model="form.{{ $field['handle'] }}" id="{{ $key }}" value="{{ $key }}" class="form-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
        <label for="{{ $key }}" class="ms-2 text-sm font-medium text-gray-900">{{ $label }}</label>
    </div>
    @endforeach

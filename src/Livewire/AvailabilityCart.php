...
class AvailabilityCart extends Component
{
    ...
    {
        if (session()->has('resrv-cart')) {
            $this->cart->fill(session('resrv-search'));
        }
    }
    ...
    public function addToCart(string $entryId, array $availabilityData, array $results): void
    {
        ...
        $isDuplicate = $this->cart->items->contains(function ($item) use ($entryId, $availabilityData) {
        ...
        if ($isDuplicate) {
            $cartId = $this->cart->items->firstWhere('entryId', $entryId)->id;
            // Update instead of adding a new item
            $this->updateCartItem($this->buildCartItemData($entryId, $availabilityData, $results, $cartId));
            return;
        }

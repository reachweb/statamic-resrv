...
class ExtraFrontTest extends TestCase
{
    ...
    public function test_can_index_extras_with_conditions()
    {
        $this->signInAdmin();
        $extra = Extra::factory()->relative()->create();
        $condition = ExtraCondition::factory()->requiredReservationTimeAndShow()->create([
            'extra_id' => $extra->id,
        ]);
        $item = $this->makeStatamicItem();
        ...
        $response = $this->post(route('resrv.extra.index'), $checkoutRequest);
        $response->assertStatus(200)->assertSee($extra->slug)->assertSee('75')->assertSee('conditions')->assertSee('pickup_time');
    }
}
